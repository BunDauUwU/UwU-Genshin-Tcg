...
class Gcg(QMainWindow):
    ...
    def handle_recv(self, datagram: bytes):
        ...
        if message == "init_character":
            ...
            if self.player_chose_deck is not None:
                deck = self.player_chose_deck[list(self.player_chose_deck.keys())[0]]
                deck_message = {"message": "check deck", "character": deck["character"], "card": deck["card"], "camp": deck["camp"]}
                deck_message = str(deck_message)
                self.socket_send(deck_message)
            ...
            if self.commit_button.isHidden():
        ...
        elif message == "remove_oppose_support":
    ...
    def start_game(self):
        if self.player_chose_deck is None:
            QMessageBox.about(self, "Prompt", "No desk")
        else:
...
class CommitButton(QPushButton):
    ...
    def update_text(self):
        if self.game.choose_target_type:
            self.setText("Choose target")
        else:
            if self.game.action_state == "play_card":
                self.setText("play_card")
            elif self.game.action_state == "element_tuning":
                self.setText("Elemental reaction")
...
class CharacterCard(QFrame):
    ...
    def change_die_picture(self):
        character_name = self.name.replace(" ", "")
        picture = QPixmap("./resources/die/%s.png" % character_name)
        self.picture.setPixmap(picture)

    def change_hp(self, hp: int):
        self.hp.setText(str(hp))
        if hp == 0:
            self.change_die_picture()
...
class Reroll(QWidget):
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        if isinstance(obj, ClickableLabel):
            self.scroll_start_state = obj.get_state() 
...
class SupportZone(QWidget):
    ...
    def remove_widget(self, index):
        remove = self.contain_widget[index]
        remove.deleteLater()
        self.contain_widget.pop(index)
        self.lo.itemAt(index).widget().deleteLater()
        for index, widget in enumerate(self.contain_widget):
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        if isinstance(obj, QLabel):
            select = obj.parent()
            if isinstance(select, SupportCard):
                if self.enable_choose_target:
                    if select in self.contain_widget:
                        if select.be_chose:
                            self.game.choose_target_index = self.contain_widget.index(select)
                        else:
...
class SummonZone(QWidget):
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        if isinstance(obj, QLabel):
            select = obj.parent()
            if isinstance(select, SummonCard):
                if self.enable_choose_target:
                    if select in self.contain_widget:
                        if select.be_chose:
                            self.game.choose_target_index = self.contain_widget.index(select)
